...
class MJS2:
    ...
    def __init__(self, robot_ns,  robot_model):
        ...
        policy_file = os.path.join(BASE_DIR, "policy", robot_ns, self.cfg["policy_files"][args.policy])
        ...
        # Observation buffer
        self.obs_buf = utils.ObservationBuffer(self.cfg["num_obs"], len(self.cfg["obs_history_ids"]))
    ...
    def reset_sim(self):
        ...
        self.obs = np.zeros(self.cfg["num_obs"] * len(self.cfg["obs_history_ids"]))
        self.obs_buf.reset()
        self.iter_ = 0

    def get_joint_state(self):
        """Re(trieve the joint position and velocity states."""
        q_ = self.data.qpos
        dq_ = self.data.qvel
        return q_[7:], dq_[6:]
    ...
    def compute_obs(self):
        ...
        # clip observation
        current_obs = np.clip(current_obs, -self.cfg["clip_obs"], self.cfg["clip_obs"])
        # get observation
        self.obs = self.obs_buf.insert(current_obs)
        self.obs = self.obs_buf.get_obs_vec(self.cfg["obs_history_ids"])
        # if add noise to obs
        if self.cfg["add_noise"]:
            self.add_noise_to_obs()

    def add_noise_to_obs(self):
        noise_vec = np.random.rand(*self.obs.shape)
        ...
        try:
            while self.data.time < 100.0 and self.viewer.is_running():
                ...
                if not self.PAUSE:
                    ...
                    #  self.data.ctrl = self.action * self.action_scale + self.joint_pos_default
...
def main():
    sim = MJS2(robot_ns="A1_himloco", robot_model="A1")
    sim.run()
